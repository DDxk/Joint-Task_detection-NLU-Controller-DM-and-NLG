...
from atis_entity_recognition.filter_parten import FilterRule
from slot_value_check.slot_value_check import SlotValueCheck, SlotLazyMapping
from atis_entity_recognition.bi_lstm_model import Bi_LSTM_Crf
from atis_entity_recognition.data_pro.data_parse import DataParse
...
class NLU(object):
    def __init__(self):
        ...
        self.word2id = pickle.load(open('data_parse/vocb', mode='rb'))  # 获取本地存放的字典。
        self.tang2index = pickle.load(open('data_parse/slot2id_id2sot', mode='rb'))['slot2id']
        self.index2tang = pickle.load(open('data_parse/slot2id_id2sot', mode='rb'))['id2slot']
        self.intent2id = pickle.load(open('data_parse/intent2id_id2intent', mode='rb'))['intent2id']
        self.id2intent = pickle.load(open('data_parse/intent2id_id2intent', mode='rb'))['id2intent']
        ...
        self.model, self.sess = self.init()
    ...
    def rwa_to_slots(self, raw_text):
        ...
        for d in bad_slots:
            marged_slot.pop(d)
        res = {}
        res.update({'task_id': task})
        res.update({'slots': marged_slot.keys()})
        res.update({'slots_values': marged_slot.values()})
        res.update({'intent': intent})
        return res


if __name__ == '__main__':  # y:m:d   h:m:s
    ...
    while True:
        ...
        if len(row_t) == 0:
            break
        print(train_pro.rwa_to_slots(row_t))
