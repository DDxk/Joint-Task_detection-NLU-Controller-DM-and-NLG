...
class DataParse:
    def __init__(self, train_data_path, slot2id_id2sot,
                 intent2id_id2intent, sentence_slot_intent_path, vocb_path):

        self.train_data_path = train_data_path  # 新的数据
        self.slot2id_id2sot = slot2id_id2sot
        self.intent2id_id2intent = intent2id_id2intent
        self.sentence_slot_intent_path = sentence_slot_intent_path
...
class Data_Inter:
    ...
    生成训练数据
    """
    def __init__(self, batch_size, sentence_slot_intent_path, slot2id_id2sot, intent2id_id2intent, vocb_path):
        self.sentence_slot_intent_path = sentence_slot_intent_path
        self.slot2id_id2sot = slot2id_id2sot
        self.intent2id_id2intent = intent2id_id2intent
        self.vocb_path = vocb_path
...
if __name__ == '__main__':
 so = DataParse(train_data_path='nlus_texts/zhibo/zhibo_texts_parse',
                slot2id_id2sot='nlus_texts/zhibo/slot2id_id2sot',
                intent2id_id2intent='nlus_texts/zhibo/intent2id_id2intent',
                sentence_slot_intent_path='nlus_texts/zhibo/sentence_slot_intent',
                vocb_path='nlus_texts/zhibo/vocb')
 so.read_corpus(so.train_data_path)

 row_datas = open(so.train_data_path, mode='r', encoding='utf-8', errors='ignore').readlines()
 so.creat_vocab(row_datas)
 # so.old_creat_vocab(row_datas)